...
<script language="javascript">
	function exportExcel(){
		window.location.href="toExcelyc";
	}
</script>
 </head>
<body>
...
PADDING-TOP: 10px; text-align:center;" class="include">
    	  ...
    	  <tr>
    	    <td width="70%"></td>
    	    <td><a href="ycSearchMX" style="text-decoration:underline;">查看明细</a></td>    	  	
    	...
    	<table class=gridView id="table_<%=(sumitems/10)+1 %>" 
	      ...
	      cellSpacing=0 rules=all border=1>
    		...
    		<tr style="font-weight: bolder">
                  ...
                  <TD class=gridViewHeader style="WIDTH: 10%" scope=col>异常次数</TD>
                  <TD class=gridViewHeader style="WIDTH: 10%" scope=col>最后一次异常时间</TD>
    		...
    		<TR onmouseover="if(choose!=<%=sumitems %>)this.style.backgroundColor = '#e2f9ff';" 
                	...
                	id="news<%=sumitems %>">
    			<td class=gridViewItem><%=ycList.get(j).getBaseId() %>&nbsp;</td>
    			<td class=gridViewItem id="title<%=sumitems %>"><%=ycList.get(j).getCategory()%>&nbsp;</td>
    			<td class=gridViewItem id="title<%=sumitems %>"><%=ycList.get(j).getBaseName()%>&nbsp;</td>
    			<td class=gridViewItem id="title<%=sumitems %>"><%=ycList.get(j).getStaffName()%>&nbsp;</td>
    			<td class=gridViewItem id="title<%=sumitems %>"><%=ycList.get(j).getYclxName() %>&nbsp;</td>
                <td class=gridViewItem id="title<%=sumitems %>"><%=ycList.get(j).getYcNum() %>&nbsp;</td>
    			<td class=gridViewItem id="title<%=sumitems %>"><%=ycList.get(j).getDate() %>&nbsp;</td>
    			<td class=gridViewItem id="title<%=sumitems %>"><%=ycList.get(j).getTable() %>&nbsp;</td>
    		</tr>
    		<%if(sumitems%10==0){ %>
    	</table>
    	<%} %>

...
<script language="javascript">
	function exportExcel(){
		window.location.href="toExcelycMX";
	}
</script>
...
<div style="position:absolute;left:0px;"> 
   ...
   &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<font style="color:#419224;font-weight:bolder;font-size:30px;">异常检测查询结果</font>
</div>
...
PADDING-TOP: 10px; text-align:center;" class="include">
    	  ...
    	  <tr>
    	    <td><a href="ycSearchchang" style="text-decoration:underline;">返回</a></td>
    	  	<td width="70%"></td>
    	...
    	<table class=gridView id="table_<%=(sumitems/10)+1 %>" 
	      ...
	      cellSpacing=0 rules=all border=1>
    		...
    		<tr style="font-weight: bolder">
                  ...
                  <TD class=gridViewHeader style="WIDTH: 10%" scope=col>异常参数</TD>
                  <TD class=gridViewHeader style="WIDTH: 10%" scope=col>异常值</TD>
                  <TD class=gridViewHeader style="WIDTH: 10%" scope=col>异常时间</TD>
                  <TD class=gridViewHeader style="WIDTH: 20%" scope=col>所属表</TD>
    		</tr>
                 <%}
                	sumitems++;
                	if(ycList.get(i).getBaseId()!=null&&ycList.get(i).getBaseId()!="")
                    {
                 %>
    		<TR onmouseover="if(choose!=<%=sumitems %>)this.style.backgroundColor = '#e2f9ff';" 
                	...
                	id="news<%=sumitems %>">
    			<td class=gridViewItem><%=ycList.get(i).getBaseId() %>&nbsp;</td>
    			<td class=gridViewItem id="title<%=sumitems %>"><%=ycList.get(i).getCategory()%>&nbsp;</td>
    			<td class=gridViewItem id="title<%=sumitems %>"><%=ycList.get(i).getBaseName()%>&nbsp;</td>
    			<td class=gridViewItem id="title<%=sumitems %>"><%=ycList.get(i).getStaffName()%>&nbsp;</td>
    			<td class=gridViewItem id="title<%=sumitems %>"><%=ycList.get(i).getYclxName() %>&nbsp;</td>
                <td class=gridViewItem id="title<%=sumitems %>"><%=ycList.get(i).getYccsName()%>&nbsp;</td>
                 <td class=gridViewItem id="title<%=sumitems %>"><%=ycList.get(i).getYczName() %>&nbsp;</td>
    			<td class=gridViewItem id="title<%=sumitems %>"><%=ycList.get(i).getDate() %>&nbsp;</td>
    			<td class=gridViewItem id="title<%=sumitems %>"><%=ycList.get(i).getTable() %>&nbsp;</td>
    		</tr>
    		<%}
